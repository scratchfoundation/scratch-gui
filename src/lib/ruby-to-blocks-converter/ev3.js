        ...
        if ((this._isSelf(receiver) || receiver === Opal.nil) && !rubyBlock) {
            ...
            case 'ev3_motor_turn_this_way_for':
                if (args.length === 2 && this._isString(args[0]) && this._isNumberOrBlock(args[1])) {
                    ...
                    const motor = Ev3MotorMenu.indexOf(args[0]._value);
                    args[0]._value = motor + '';
                    this._addInput(block,
                    'PORT',
                    this._createFieldBlock('ev3_menu_motorPorts', 'motorPorts', args[0]));
            ...
            case 'ev3_button_pressed?':
                if (args.length === 1 && this._isString(args[0])) {
                    ...
                    const sensor = Ev3SensorMenu.indexOf(args[0]._value);
                    args[0]._value = sensor + '';
                    this._addInput(block,
                    'PORT',
                    this._createFieldBlock('ev3_menu_sensorPorts', 'sensorPorts', args[0]));
                }
                break;
            case 'ev3_distance':
                if (args.length === 0) {
